...
function clicked(e) {
    ...
    if (selected_node.asset_type == "Actor" || selected_node.asset_type == "External Entity") {
        bottom_bar_html += "<br><h3>Associated Trust Boundary</h3>";
        if (selected_node.boundaries.length != 0) {
            bottom_bar_html += "<div id=\"display_associated\">";
            for (let key of selected_node.boundaries) {
                bottom_bar_html += "<span style=\"cursor: pointer\" class=\"assoc_bound\">" + key + "</span>";
                if (key != selected_node.boundaries[selected_node.boundaries.length - 1]) {
                    bottom_bar_html += "<br>";
                }
            }
        }
        else {
            bottom_bar_html += "<div id=\"display_associated\">No associated trust boundary...";
        }
    ...
    var assocs = document.getElementsByClassName("assoc_bound");
    ...
    if (selected_node.asset_type == "Actor" || selected_node.asset_type == "External Entity") {
        associateBoundary(selected_node);
        document.getElementById("edit_assoc").onclick = function() {
            editAssociated(selected_node);
        }
    }
...
function editAssociated(asset) {
    ...
    div.innerHTML = "";
    for (let key of asset.boundaries) {
        div.innerHTML += "<button class=\"unassoc_bound\">&#10006" + key + "</button>";
        if (key != asset.boundaries[asset.boundaries.length - 1]) {
            div.innerHTML += ",  ";
        }
    }
    document.getElementById("edit_assoc").style.display = "none";
    document.getElementById("associate_trust").style.display = "none";
    document.getElementById("done_associate").style.display = "none";
    div.innerHTML += "<br><button id=\"done_unassoc\">Done</button>"; 
    document.getElementById("done_unassoc").onclick = function () {
        svg.selectAll(".node_group").filter(d => d === asset).dispatch('click').dispatch('click');
    }
    for (let i = asset.boundaries.length - 1; i >= 0; i--) {
        document.getElementsByClassName("unassoc_bound")[i].onclick = function () {
            var bound = asset.boundaries[i];
            if (!confirm("Unassociate " + bound + " from " + asset.asset_name + "?")) {
                return;
            }
            var assets = boundaries[bound];
            if (assets.length == 1) {
                if (!confirm("This will delete " + bound + " because " + asset.asset_name + " is the last asset in the trust boundary. Continue?")) {
                    return;
                }
            ...
            if (!checkConnected(assets)) {
                ...
                if (!confirm("This will delete " + bound + " because all assets must be connected. Continue?")) {
                    return;
                }
                deleteBoundary(bound);
                // Unselect all nodes
            ...
            svg.selectAll(".link_group").filter(d => (d.source == removed && !assets.includes(d.target)) || (d.target == removed && !assets.includes(d.source))).selectAll(".boundary").filter(function() { return d3.select(this).attr("boundaryName") == bound; }).remove();
...
function deleteBoundary(boundary_name) {
    var assets = boundaries[boundary_name];
    svg.selectAll(".boundary").each(function(d) {
        if (boundary_name == d3.select(this).attr("boundaryName")) {
...
function associateBoundary(node) {
    const boundary_names = Object.keys(boundaries);
    const display = document.getElementById("display_associated");
    const dropdown = document.getElementById("associate_trust");
    const done = document.getElementById("done_associate");
    updateBoundaryDropdown();
    if (node.boundaries.length === boundary_names.length) {
    ...
    for (let i = Object.keys(boundaries).length - 1; i >= 0; i--) {
        if (node.boundaries.includes(boundary_names[i])) {
            dropdown.remove(i + 1);
        }
    }
    done.onclick = function () {
        ...
        node.boundaries.push(boundary_name);
        display.innerHTML += "<span style=\"cursor: pointer\" class=\"assoc_bound\">" + boundary_name + "</span>";
        const assocs = document.getElementsByClassName("assoc_bound");
        for (let i = 0; i < assocs.length; i++) {
            assocs[i].onclick = function() {
                resetBottomBar();
        ...
        const flow = d3.selectAll(".link_group").filter(d => (assets.includes(d.source) && !assets.includes(d.target)) || (assets.includes(d.target) && !assets.includes(d.source)));

        // If there are inter-trust boundaries, retrieve jitter value, else 
        // generate a new one
        let jitter;
        if (d3.selectAll(".boundary").filter(function() {return d3.select(this).attr("boundaryName") === boundary_name;}).empty()) {
            jitter = (Math.random() * 0.3) + 0.1;
        }
        else {
            jitter = d3.selectAll(".boundary").filter(function() {
...
function updateBoundaryDropdown() {
    let boundary_dropdowns = document.getElementsByClassName("boundary_dropdown");

    let view_button = document.querySelector("#boundary_view");
    ...
    if (Object.keys(boundaries).length === 0) {
        ...
        for (let boundary_dropdown of boundary_dropdowns) {
            ...
            if (view_button) { view_button.disabled = false; }
...
function viewBoundary() {
    ...
    bottom_bar_html += "</h3><br><h3>Associated Actors</h3>";
    var actors = assets.filter(d => d.asset_type == "Actor");
    for (let actor of actors) {
        bottom_bar_html += "<span>" + actor.asset_name + "</span>";
        if (actor != actors[actors.length - 1]) {
            bottom_bar_html += ",  ";
        }
    }
    if (actors.length == 0) {
        bottom_bar_html += "<span>No associated actors...</span>";
    }

    bottom_bar_html += "<br><h3>Associated External Entities</h3>";
    var externs = assets.filter(d => d.asset_type == "External Entity");
    for (let extern of externs) {
        bottom_bar_html += "<span>" + extern.asset_name + "</span>";
        if (extern != externs[externs.length - 1]) {
            bottom_bar_html += ",  ";
        }
    }
    if (externs.length == 0) {
        bottom_bar_html += "<span>No associated external entities...</span>";
    }
    bottom_bar_html += "<br> <br> <button type=\"button\" class=\"view_button\" id=\"edit_button\">Edit assets</button>";
...
function editBoundary(selectedBoundary) {
    ...
    // Allow user to stop editing the boundary
    document.getElementById("done_button").onclick = function () {
        resetBottomBar();
